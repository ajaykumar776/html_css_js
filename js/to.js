$(document).ready(function(){       /*now , our document is ready*/
    var todos = [];                 /*Have taken a blank array, for storing all li tags.*/
    ...
    function todo_add_ajax(){       
        var todo = $("#todo_input").val();          /*after pressing the button, or enter key, the program will come here.here we are simply pushing the text in todos array.so, using the id of the input tag,we are taking its value and storing it in var. */
        if(todo){                   /*so if our text will true only , then it will proceed.suppose anyone has not written anything, i.e a blank char any entered, then it should not append in our array. */
            ...
            $("#todo_input").val("");           /*after adding in array, and pushing it in ul tag, now in our input tag, it should be empty, so by taking id of the input tag , we are changing its value to blank char. */
        }
    }

    function  todo_add(){       /*function is not taking any parameters.and not returning any thing. */
        $("#todo_btn").click(function(e){           /*so, if anyony will click on the button, then we have to add the text in todos array.so here, we took the id of the button, and if anyone will click,then it will call a function, */
    ...
    function todo_delete(){         /*defined a funciton */
        $(document).on("click", ".todo_delete_btn", function(){     /*in a li tag, we have inserted our text as well as button.so when any one will click on that x button, our list should be delete from there.so here , we are taking the id of the button, and on clicking , we are calling a call back function! */
            ...
            if(todo_alert === true){                                            /*it will go inside, if user has conformed else will not go. */
                var todo = $(this).parent().find(".todo_text").text()           /*take your mouse cursor to todos wbesite, and hold your cursor on the x button. and then press ctrl+shift+i to inspect the codes.then in the same page, one more window will open, and in elements,you will get a html codes.so then go to the ul tag, you will see there inside ul, there are lots of li tags.so basically we want the text value.thats why firstly we have gone to parent and then finds the value. */
                console.log(todo)
    ...
    init();     /*Our Program starts from here, we have called a function named init().we are not passing any parameters.*/
});